...
public class WorkerInventario_OrdineCreato : BackgroundService
{
    private readonly ILogger<WorkerInventario_OrdineCreato> _logger;
                            ...
                            IConnection connection)
    {
        ...
        _Queue_Read_Ordine_DLQ = PARAMETRI.QUEUE.PROPRIETA.ORDINI.NAME_DLQ;
        ...
        argumentsToDle_Ordine = new Dictionary<string, object>
        {
            {"x-dead-letter-exchange", _Queue_Read_Ordine_DLQ}
        };
    ...
    private void DichiarazioneQueue_Read_From_Ordini()
    {
        ...
        _channel.QueueDeclare(queue: _Queue_Read_Ordine,
                              ...
                              arguments: argumentsToDle_Ordine);


        //SPOSTAMENTI EVENTI IN DLQ
        //indicazione della coda specifica per il servizio di creazione ordini a partire da richiesta creazione ordini
        _logger.LogInformation("DEFINIZIONE CODA DL (QUEUE) {0}",
                            _Queue_Read_Ordine_DLQ);
        _channel.QueueDeclare(queue: _Queue_Read_Ordine_DLQ,
                              durable: PARAMETRI.QUEUE.PROPRIETA.DURABLE,
                              exclusive: PARAMETRI.QUEUE.PROPRIETA.ESCLUSIVE,
                              autoDelete: PARAMETRI.QUEUE.PROPRIETA.AUTODELETE,
                              arguments: null);
    ...
    private void AssociazioneQueueESottoscrizioneExchange()
    {
        ...
        _logger.LogInformation("ASSOCIAZIONE QUEUE {0} A EXCHANGE {1} e Sottoscrizione evento {2}",
                        _Queue_Pubblicazione_Inventario,
                        _Queue_Exchange,
                        PARAMETRI.QUEUE.KEY_ROUTING_EVENTO.INVENTARIO.PROCESSATO.ALLOCATA);

        _channel.QueueBind(queue: _Queue_Pubblicazione_Inventario,
                           exchange: _Queue_Exchange,
                           routingKey: PARAMETRI.QUEUE.KEY_ROUTING_EVENTO.INVENTARIO.PROCESSATO.ALLOCATA,
                              arguments: null);
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        if (_channel == null)
        {
            _logger.LogError("Canale RabbitMQ non inizializzato. il Worker Ordine (Inventario) non può avviarsi.");
            return;
        }
    ...
    //            IdSaga = evento.IdSaga.ToString(),
    //        };
    //        await PubblicazioneEvento(PARAMETRI.QUEUE.KEY_ROUTING_EVENTO.INVENTARIO.PROCESSATO.RIALLOCATA, eventoCompensazione);
